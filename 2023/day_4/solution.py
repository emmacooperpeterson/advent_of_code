...
def get_card_number(card):
    # card is one line of input
    return int(re.search("^Card\s*(\d*):", card).group(1))

def get_winning_numbers(card):
    number_string = re.search("^Card\s*\d*:\s(.*)\|", card).group(1)
    number_set = set(" ".join(number_string.split()).split(" "))
    return number_set

def get_my_numbers(card):
    number_string = re.search(".*\|(.*)$", card).group(1)
    number_set = set(" ".join(number_string.split()).split(" "))
    return number_set
...
def get_num_matches(card):
    matches = card["winning_numbers"].intersection(card["my_numbers"])
    num_matches = len(matches)
    return {"card_number": card["card_number"], "num_matches": num_matches}

def get_points(card):
    num_matches = card["num_matches"]
    points = 0 if num_matches == 0 else 1 * 2**(num_matches-1)
    return points


matches = [get_num_matches(card) for card in cards_clean]
total_points = sum(get_points(card) for card in matches)
...
# create a map of {card: [copies], ...}
# e.g. {1: [2, 3], 2: [0], 3: [4, 5, 6]}
# copies is based on the number of winning numbers for that card
# e.g. if card 3 has 3 winning numbers, the copies are [4, 5, 6]
copy_map = {}
for match in matches:
    ...
    copies = matches[card_number:card_number+num_matches]
    copy_map[card_number] = [c["card_number"] for c in copies]

# reformat matches to be just a list of card numbers
card_numbers = [m["card_number"] for m in matches]

# for each card, look up the copies in copy_map and append them to the end
for card_number in card_numbers:
    cards_to_add = copy_map[card_number]
    card_numbers += cards_to_add

result = len(card_numbers)
